...
#the whole problem is that the datesats are not equal. BTW i am a moron, 
# just had to drop the attributes and keep only the ones of forest no forest and the geom.

#Extract Vector with the names of the PNN 
...
#load mask 
 ...
 setwd('/Users/sputnik/Documents/bosque-nobosque')

 
 # crop the template raster
# # for some reason future_map is not working in any machine. Find out why!
...
tiffes <- list.files('.', pattern='harm_bin') # double check this!
#select the specific years
harm <-list()
for(i in 1:length(tiffes)){
  harm[i] <- raster(tiffes[i])}

#crop and mask for each of the PNN. for rasterize. (name_pmm.tif)
...
#datos PNN rasterizados por lugar y por año. Paso internedio. Esto hay que hacerlo en nested map                         
...
# I have to be able to run this as a single thing, but have not been successful yet. 
#Extract maps 
#Set  reck 
...
#crop/mask 
#No he podido. Mapas harmonizados por lugar y por año. Esto lo tengo que poder hacer en mapa
...
# load libraries to run comparisons

library(diffeR)
library(greenbrown)

#set function from diffeR to map
...
# new cols
vect_1 <- c(c(rep(2002, times=12)), c(rep(2007, times=12)))


years. <- c(2002,2007,2012,2015,2017,2018)


#vect_1 <- as_tibble(vect_1)
# rbind all tables
...
col_names <- c('class', 'no-bosque', 'bosque', 'year', 'pnn')
...
# ##################################
# 
# #Produce aGreement Maps with greebrown
...
# map(1:length(harm6), function(x) writeRaster(harm6[[x]],paste('harm6_bin', years.[x], sep='_'), overwrite=TRUE)) 
# 
# 
# 
# years.
